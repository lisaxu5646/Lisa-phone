...
- P1-1：分类与证据逐字核验 shadow 已运行；**对照基线并非纯旧版**：真实抽取 prompt 已带消息 ID、逐字证据要求与“找不到别造”的证据闸，但分类/temperature 的采纳规则仍走旧路。转正评审必须按“已带证据闸的 baseline”解释，不能宣称与改造前完全同路。
- P1-2：`memory_surface_state_*.sql` 仅为 dormant 设计/测试包，**未部署**；App 也没有状态写入口。
- P1-3：2026-07-18 已部署并在 App v49.60 转正：候选/双行原子决定/审计/回滚测试全在位；`pruneSubsumed` 不再硬删，确定性包含关系只提候选，Lisa 过目确认后旧条才标 `superseded`，两行永久保留。
...
- P2 评审口径：v49.27 后自动闭环写路已删除，open 在 RepairGate 转正前持续堆积是预期行为，不得把“未自动关闭”误报为异常。
...
- 有效消息分支：v49.31 旁路记录编辑、撤回、删除、重生成前后的条数和结构；不存正文、不改历史。重点识别“重生成旧回复却保留依赖旧回答的后续消息”这一悬空尾巴风险。
- 独立洞察候选：v49.32 复用现有抽取结果中的 `kind=insight`，旁路检查结论、推导、转折、逐字金句是否齐全，并统计洞察混入普通记忆的比例；不另叫 AI、不存正文、暂不建权威洞察表。
